    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.payments.update", [$payment->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('client_name') ? 'is-invalid' : '' }}" name="client_name_id" id="client_name_id">
                    @foreach($client_names as $id => $entry)
                        <option value="{{ $id }}" {{ (old('client_name_id') ? old('client_name_id') : $payment->client_name->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('product_name') ? 'is-invalid' : '' }}" type="text" name="product_name" id="product_name" value="{{ old('product_name', $payment->product_name) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('invoice') ? 'is-invalid' : '' }}" type="text" name="invoice" id="invoice" value="{{ old('invoice', $payment->invoice) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('amount_paid') ? 'is-invalid' : '' }}" type="text" name="amount_paid" id="amount_paid" value="{{ old('amount_paid', $payment->amount_paid) }}">
